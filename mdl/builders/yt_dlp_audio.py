...
def build_audio_command(url: str, opts: RunOptions) -> List[str]:
    ...
    if is_playlist:
        mode = str(getattr(opts, "playlist_mode", "auto")).strip().lower()
        if mode == "album":
            layout = "album"
        elif mode == "flat":
            layout = "flat"
        else:
            layout = detect_playlist_layout(url)
        tpl = Defaults.audio_playlist_tpl if layout == "album" else Defaults.audio_playlist_flat_tpl
    else:
        tpl = Defaults.audio_single_tpl
    ...
    if is_playlist:
        # Normalize autogenerated labels like "Album - <name>" to "<name>".
        cmd += ["--replace-in-metadata", "playlist_title", r"^Album -\s*", ""]
